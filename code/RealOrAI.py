...
# 테스트 -----------------------------------------------------------------------
def preprocess_uploaded_image(path, size=(128, 128)):
    img = Image.open(path).convert("RGB")     # RGB로 강제 변환
    img = img.resize(size)                    # 크기 맞추기
    img = np.array(img, dtype=np.float32) / 255.0  # 정규화
    img = np.expand_dims(img, axis=0)         # (1, 128, 128, 3)로 차원 맞춤
    return img

img = preprocess_uploaded_image(r'C:\Users\admin\OneDrive\Gambar\fake-test3.jpg')
pred = model.predict(img)
print("AI 확률:", pred[0][0])

if pred[0][0] >= 0.5:
    print("결과: AI 그림으로 판별됨")
else:
    print("결과: 명화로 판별됨")
